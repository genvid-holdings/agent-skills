...
`register_media` binds a file to **one shot or one asset**, and its slots are typed: a **shot** slot (`shot_keyframe` / `shot_firstframe` / `shot_lastframe` for a still frame, `shot_video` for moving-image) or an **asset** slot. Asset slots come in three families, one per asset type: `{asset_type}_image` for a still, `{asset_type}_video` for moving-image, and `{asset_type}_model` for 3D geometry — so `location_image`, `location_video`, `location_model`, and the same for every other asset type. Dialog is the exception: `cast_member_dialog` is the only audio slot on any asset, because only a cast member speaks. A scene is not a direct registry slot — it is captured in the proposed index for review and is expressed through the shot that lives in it or the asset the file depicts. **Match the slot to the file's media kind: a video original goes in a video slot, a still goes in an image slot** — the boundary does not reject a video bound into an image slot, so getting this right is on you. Resolve each approved binding this way:
...
- **Reference / character / location media → the asset it depicts.** `asset_id` + the slot matching both the asset's type and the file's media kind: `cast_member_image` for a character still, `location_image` for a location still, `location_video` for location footage, `prop_model` for a prop mesh, and so on. The `link_type` must agree with the asset's type on the prefix and with the file's media kind on the suffix.
- **Camera / dailies clip.** The binding contract anchors it to its **scene** and that scene's **physical-location** asset in the *proposed index* — that records what it depicts, for the human. A raw dailies clip is a **video**, and the location asset now has a matching `location_video` slot, so bind it there rather than flagging it: the slot that once did not exist does now. Still do **not** speculatively assign it to a specific `shot_video` slot it was never cut to — dailies predate shot assignment, and that is a rebind the precision-first contract avoids. A human decides at review whether it ties to a known shot. Where the depicted asset is not a location, or you cannot resolve which asset it depicts, `flag_hitl` as before.
