...
- **Camera / dailies clip.** The binding contract anchors it to its **scene** and that scene's **physical-location** asset in the *proposed index* — that records what it depicts, for the human. A raw dailies clip is a **video**, and the location asset has a matching `location_video` slot, so bind it there rather than flagging it. Still do **not** speculatively assign it to a specific `shot_video` slot it was never cut to — dailies predate shot assignment, and that is a rebind the precision-first contract avoids. A human decides at review whether it ties to a known shot. Where the depicted asset is not a location, or you cannot resolve which asset it depicts, `flag_hitl`.
